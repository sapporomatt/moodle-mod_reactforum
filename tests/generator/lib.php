...
/**
 * ReactForum module data generator class
 *
 * @package    mod_reactforum
 * @category   test
 * @copyright  2012 Petr Skoda {@link http://skodak.org}
...
class mod_reactforum_generator extends testing_module_generator {
    ...
    /**
     * @var int keep track of how many reactforum discussions have been created.
     */
    protected $reactforumdiscussioncount = 0;

    /**
     * @var int keep track of how many reactforum posts have been created.
     */
    protected $reactforumpostcount = 0;

    /**
     * @var int keep track of how many reactforum subscriptions have been created.
     */
    protected $reactforumsubscriptionscount = 0;
    ...
    public function reset() {
        $this->reactforumdiscussioncount = 0;
        $this->reactforumpostcount = 0;
        $this->reactforumsubscriptionscount = 0;
    ...
    public function create_instance($record = null, array $options = null) {
        ...
        if (!isset($record->forcesubscribe)) {
            $record->forcesubscribe = REACTFORUM_CHOOSESUBSCRIBE;
        }
    ...
    public function create_subscription($record = null) {
        ...
        // Increment the reactforum subscription count.
        $this->reactforumsubscriptionscount++;
        ...
        if (!isset($record['reactforum'])) {
            throw new coding_exception('reactforum must be present in phpunit_util::create_subscription() $record');
        }
        ...
        $record->id = $DB->insert_record('reactforum_subscriptions', $record);
    ...
    public function create_discussion($record = null) {
        ...
        // Increment the reactforum discussion count.
        $this->reactforumdiscussioncount++;
        ...
        if (!isset($record['reactforum'])) {
            throw new coding_exception('reactforum must be present in phpunit_util::create_discussion() $record');
        }
        ...
        if (!isset($record['name'])) {
            $record['name'] = "Discussion " . $this->reactforumdiscussioncount;
        }

        if (!isset($record['subject'])) {
            $record['subject'] = "Subject for discussion " . $this->reactforumdiscussioncount;
        }

        if (!isset($record['message'])) {
            $record['message'] = html_writer::tag('p', 'Message for discussion ' . $this->reactforumdiscussioncount);
        }
        ...
        if (isset($timemodified) || isset($mailed)) {
            ...
            if (isset($timemodified)) {
                ...
                $DB->update_record('reactforum_discussions', $record);
            }
    ...
    public function create_post($record = null) {
        ...
        // Increment the reactforum post count.
        $this->reactforumpostcount++;

        // Variable to store time.
        $time = time() + $this->reactforumpostcount;
        ...
        if (!isset($record['subject'])) {
            $record['subject'] = 'ReactForum post subject ' . $this->reactforumpostcount;
        }

        if (!isset($record['message'])) {
            $record['message'] = html_writer::tag('p', 'ReactForum message post ' . $this->reactforumpostcount);
        }
    ...
    public function create_content($instance, $record = array()) {
        ...
        if (empty($record['discussion']) && empty($record['parent'])) {
            ...
            $post = $DB->get_record('reactforum_posts', array('id' => $discussion->firstpost));
        } else {
            // Create post.
            if (empty($record['parent'])) {
                $record['parent'] = $DB->get_field('reactforum_discussions', 'firstpost', array('id' => $record['discussion']), MUST_EXIST);
            } else if (empty($record['discussion'])) {
                $record['discussion'] = $DB->get_field('reactforum_posts', 'discussion', array('id' => $record['parent']), MUST_EXIST);
            }
