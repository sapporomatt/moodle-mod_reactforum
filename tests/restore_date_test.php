 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_reactforum_restore_date_testcase extends restore_date_testcase {
    ...
    public function test_restore_dates() {
        ...
        $gg = $this->getDataGenerator()->get_plugin_generator('mod_reactforum');
        $record = ['assesstimefinish' => 100, 'assesstimestart' => 100, 'ratingtime' => 1, 'assessed' => 2, 'scale' => 1];
        list($course, $reactforum) = $this->create_course_and_module('reactforum', $record);

        // ReactForum Discussions/posts/ratings.
        $timestamp = 996699;
        $diff = $this->get_diff();
        $record = new stdClass();
        ...
        $DB->set_field('reactforum_discussions', 'timemodified', $timestamp);

        // Ratings.
        $ratingoptions = new stdClass;
        $ratingoptions->context = context_module::instance($reactforum->cmid);
        ...
        $newreactforum = $DB->get_record('reactforum', ['course' => $newcourseid]);

        $this->assertFieldsNotRolledForward($reactforum, $newreactforum, ['timemodified']);
        $props = ['assesstimefinish', 'assesstimestart'];
        $this->assertFieldsRolledForward($reactforum, $newreactforum, $props);

        $newdiscussion = $DB->get_record('reactforum_discussions', ['reactforum' => $newreactforum->id]);
        $newposts = $DB->get_records('reactforum_posts', ['discussion' => $newdiscussion->id]);
        $newcm = $DB->get_record('course_modules', ['course' => $newcourseid, 'instance' => $newreactforum->id]);

        // ReactForum discussion time checks.
