...
Feature: A user can navigate to previous and next discussions
  ...
  Scenario: A user can navigate between discussions
    Given the following "activities" exist:
      ...
      | reactforum      | Test reactforum name        | Test reactforum name   | C1     | reactforum        | 0         |
  ...
  Scenario: A user can navigate between discussions with visible groups
    Given the following "activities" exist:
      | activity   | name                   | intro             | course | idnumber     | groupmode |
      | reactforum      | Test reactforum name        | Test reactforum name   | C1     | reactforum        | 2         |
    And I log in as "teacher1"
    And I follow "Course 1"
    And I follow "Test reactforum name"
    And I add a new discussion to "Test reactforum name" reactforum with:
  ...
  Scenario: A user can navigate between discussions with separate groups
    Given the following "activities" exist:
      | activity   | name                   | intro             | course | idnumber     | groupmode |
      | reactforum      | Test reactforum name        | Test reactforum name   | C1     | reactforum        | 1         |
    And I log in as "teacher1"
    And I follow "Course 1"
    And I follow "Test reactforum name"
    And I add a new discussion to "Test reactforum name" reactforum with:
