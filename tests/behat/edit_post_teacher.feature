@mod @mod_reactforum
Feature: Teachers can edit or delete any reactforum post
  In order to refine the reactforum contents
  As a teacher
  I need to edit or delete any user's reactforum posts
  ...
  Scenario: A student can't edit or delete another user's posts
    ...
    Then I should not see "Edit" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' reactforumpost ')][contains(., 'Teacher post subject')]" "xpath_element"
    And I should not see "Delete" in the "//div[contains(concat(' ', normalize-space(@class), ' '), ' reactforumpost ')][contains(., 'Teacher post subject')]" "xpath_element"
