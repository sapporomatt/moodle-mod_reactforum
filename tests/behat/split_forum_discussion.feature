@mod @mod_reactforum
Feature: ReactForum discussions can be split
  In order to manage reactforum discussions in my course
  As a Teacher
  ...
  Background:
    ...
    And I add a "ReactForum" to section "1" and I fill the form with:
      | ReactForum name | Study discussions |
      | ReactForum type | Standard reactforum for general use |
      | Description | ReactForum to discuss your coursework. |
    And I add a new discussion to "Study discussions" reactforum with:
      | Subject | Photosynethis discussion |
      | Message | Lets discuss our learning about Photosynethis this week in this thread. |
    And I log out
    And I log in as "student1"
    And I follow "Science 101"
    And I reply "Photosynethis discussion" post from "Study discussions" reactforum with:
      | Message | Can anyone tell me which number is the mass number in the periodic table? |
    And I log out

  Scenario: Split a reactforum discussion
