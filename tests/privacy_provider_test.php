...
/**
 * Tests for the reactforum implementation of the Privacy Provider API.
 *
...
use \mod_reactforum\privacy\provider;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_reactforum_privacy_provider_testcase extends \core_privacy\tests\provider_testcase {

    // Include the privacy subcontext_info trait.
    // This includes the subcontext builders.
    use \mod_reactforum\privacy\subcontext_info;
    ...
    /**
     * Helper to assert that the reactforum data is correct.
     *
     * @param   object  $expected The expected data in the reactforum.
     * @param   object  $actual The actual data in the reactforum.
     */
    protected function assert_reactforum_data($expected, $actual) {
        // Exact matches.
    ...
    public function test_user_has_never_posted() {
        // Create a course, with a reactforum, our user under test, another user, and a discussion + post from the other user.
        $course = $this->getDataGenerator()->create_course();
        $this->getDataGenerator()->create_module('reactforum', ['course' => $course->id]);
        $course = $this->getDataGenerator()->create_course();
        $this->getDataGenerator()->create_module('reactforum', ['course' => $course->id]);
        ...
        $contextlist = $this->get_contexts_for_userid($user->id, 'mod_reactforum');
        ...
        $this->export_context_data_for_user($user->id, $context, 'mod_reactforum');
        ...
        $this->assertEmpty($writer->get_data([get_string('discussions', 'mod_reactforum')]));
     ...
     * posted and has subscribed to the reactforum will have relevant
     * information returned.
     */
    public function test_user_has_never_posted_subscribed_to_reactforum() {
        global $DB;
        ...
        // Subscribe the user to the reactforum.
        ...
        $subcontext = $this->get_subcontext($reactforum);
        ...
        // There should be data about the reactforum itself.
        ...
        $this->assertCount(1, $DB->get_records('reactforum_subscriptions', ['userid' => $user->id]));
        provider::delete_data_for_user($approvedcontextlist);
        $this->assertCount(0, $DB->get_records('reactforum_subscriptions', ['userid' => $user->id]));
    }
    ...
    public function test_user_has_never_posted_subscribed_to_discussion() {
        ...
        // There should be nothing in the reactforum. The user is not subscribed there.
        $reactforumsubcontext = $this->get_subcontext($reactforum);
        $this->assertCount(0, $writer->get_all_metadata($reactforumsubcontext));
        $this->assert_reactforum_data($reactforum, $writer->get_data($reactforumsubcontext));

        // There should be metadata in the discussion.
        $discsubcontext = $this->get_subcontext($reactforum, $discussion);
        ...
        $postsubcontext = $this->get_subcontext($reactforum, $discussion, $post);
        ...
        $this->assertCount(1, $DB->get_records('reactforum_discussion_subs', ['userid' => $user->id]));
        provider::delete_data_for_user($approvedcontextlist);
        $this->assertCount(0, $DB->get_records('reactforum_discussion_subs', ['userid' => $user->id]));
    }
    ...
    public function test_user_has_posted_own_discussion() {
        ...
        list($otherdiscussion, $otherpost) = $this->helper_post_to_reactforum($reactforum, $otheruser);
        $cm = get_coursemodule_from_instance('reactforum', $reactforum->id);
        $context = \context_module::instance($cm->id);
        ...
        $this->assertEmpty($writer->get_data($this->get_subcontext($reactforum, $otherdiscussion)));

        $this->assert_discussion_data($discussion, $writer->get_data($this->get_subcontext($reactforum, $discussion)));
        $this->assert_post_data($post, $writer->get_data($this->get_subcontext($reactforum, $discussion, $post)), $writer);
    }
    ...
    public function test_user_has_posted_reply() {
        ...
        // Create several courses and reactforums. We only insert data into the final one.
        $course = $this->getDataGenerator()->create_course();
        $this->getDataGenerator()->create_module('reactforum', ['course' => $course->id]);
        $course = $this->getDataGenerator()->create_course();
        $this->getDataGenerator()->create_module('reactforum', ['course' => $course->id]);
        ...
        $discussion = $DB->get_record('reactforum_discussions', ['id' => $discussion->id]);
        $otherdiscussion = $DB->get_record('reactforum_discussions', ['id' => $otherdiscussion->id]);

        // The other discussion should not have been returned as we did not post in it.
        $this->assertEmpty($writer->get_data($this->get_subcontext($reactforum, $otherdiscussion)));

        // Our discussion should have been returned as we did post in it.
        $data = $writer->get_data($this->get_subcontext($reactforum, $discussion));
        $this->assertNotEmpty($data);
        $this->assert_discussion_data($discussion, $data);

        // The reply will be included.
        $this->assert_post_data($reply, $writer->get_data($this->get_subcontext($reactforum, $discussion, $reply)), $writer);
        ...
        $reply = $DB->get_record('reactforum_posts', ['id' => $reply->id]);
        $this->assertEmpty($reply->subject);
        $this->assertEmpty($reply->message);
        $this->assertEquals(1, $reply->deleted);

        $post = $DB->get_record('reactforum_posts', ['id' => $post->id]);
    ...
    public function test_user_has_rated_others() {
        ...
        $course = $this->getDataGenerator()->create_course();
        $reactforum = $this->getDataGenerator()->create_module('reactforum', [
            'course' => $course->id,
            'scale' => 100,
        ]);
        list($user, $otheruser) = $this->helper_create_users($course, 2);
        ...
        $this->assertEmpty($writer->get_data($this->get_subcontext($reactforum, $discussion)));

        $this->assert_all_own_ratings_on_context(
            $user->id,
            $context,
            $this->get_subcontext($reactforum, $discussion, $post),
            'mod_reactforum',
            'post',
            $post->id
        );
     ...
     */
    public function test_user_reactforum_digest() {
        global $DB;

        $course = $this->getDataGenerator()->create_course();

        $reactforum0 = $this->getDataGenerator()->create_module('reactforum', ['course' => $course->id]);
        $cm0 = get_coursemodule_from_instance('reactforum', $reactforum0->id);
        $context0 = \context_module::instance($cm0->id);

        $reactforum1 = $this->getDataGenerator()->create_module('reactforum', ['course' => $course->id]);
        $cm1 = get_coursemodule_from_instance('reactforum', $reactforum1->id);
        $context1 = \context_module::instance($cm1->id);

        $reactforum2 = $this->getDataGenerator()->create_module('reactforum', ['course' => $course->id]);
        $cm2 = get_coursemodule_from_instance('reactforum', $reactforum2->id);
        $context2 = \context_module::instance($cm2->id);

        $reactforum3 = $this->getDataGenerator()->create_module('reactforum', ['course' => $course->id]);
        $cm3 = get_coursemodule_from_instance('reactforum', $reactforum3->id);
        $context3 = \context_module::instance($cm3->id);

        list($user) = $this->helper_create_users($course, 1);

        // Set a digest value for each reactforum.
        reactforum_set_user_maildigest($reactforum0, 0, $user);
        ...
        $this->assertEquals(0, $DB->get_field('reactforum_digests', 'maildigest', ['userid' => $user->id, 'reactforum' => $reactforum0->id]));
        $this->assertEquals(1, $DB->get_field('reactforum_digests', 'maildigest', ['userid' => $user->id, 'reactforum' => $reactforum1->id]));
        $this->assertEquals(2, $DB->get_field('reactforum_digests', 'maildigest', ['userid' => $user->id, 'reactforum' => $reactforum2->id]));
        provider::delete_data_for_user($approvedcontextlist);
        $this->assertEquals(0, $DB->get_field('reactforum_digests', 'maildigest', ['userid' => $user->id, 'reactforum' => $reactforum0->id]));
        $this->assertFalse($DB->get_field('reactforum_digests', 'maildigest', ['userid' => $user->id, 'reactforum' => $reactforum1->id]));
        $this->assertEquals(2, $DB->get_field('reactforum_digests', 'maildigest', ['userid' => $user->id, 'reactforum' => $reactforum2->id]));

    }

    /**
     * Test that the per-user, per-reactforum user tracking data is exported.
     */
    public function test_user_tracking_data() {
        ...
        $reactforumoff = $this->getDataGenerator()->create_module('reactforum', ['course' => $course->id]);
        $cmoff = get_coursemodule_from_instance('reactforum', $reactforumoff->id);
        $contextoff = \context_module::instance($cmoff->id);

        $reactforumon = $this->getDataGenerator()->create_module('reactforum', ['course' => $course->id]);
        $cmon = get_coursemodule_from_instance('reactforum', $reactforumon->id);
        $contexton = \context_module::instance($cmon->id);

        list($user) = $this->helper_create_users($course, 1);

        // Set user tracking data.
        reactforum_tp_stop_tracking($reactforumoff->id, $user->id);
        reactforum_tp_start_tracking($reactforumon->id, $user->id);

        // Run as the user under test.
        $this->setUser($user);

        // Retrieve all contexts - only the reactforum tracking reads should be included.
        $contextlist = $this->get_contexts_for_userid($user->id, 'mod_reactforum');
        $this->assertCount(1, $contextlist);
        $this->assertEquals($contextoff, $contextlist->current());

        // Check export data for each context.
        $this->export_context_data_for_user($user->id, $contextoff, 'mod_reactforum');
        $this->assertEquals(0,
                \core_privacy\local\request\writer::with_context($contextoff)->get_metadata([], 'trackreadpreference'));

        // Delete the data for one of the users in the 'on' reactforum.
        $approvedcontextlist = new \core_privacy\tests\request\approved_contextlist(
            \core_user::get_user($user->id),
            'mod_reactforum',
            [$contexton->id]
        ...
        $this->assertTrue($DB->record_exists('reactforum_track_prefs', ['userid' => $user->id, 'reactforumid' => $reactforumoff->id]));
        $this->assertFalse($DB->record_exists('reactforum_track_prefs', ['userid' => $user->id, 'reactforumid' => $reactforumon->id]));
        ...
        // Delete the data for one of the users in the 'off' reactforum.
        $approvedcontextlist = new \core_privacy\tests\request\approved_contextlist(
            \core_user::get_user($user->id),
            'mod_reactforum',
            [$contextoff->id]
        ...
        $this->assertFalse($DB->record_exists('reactforum_track_prefs', ['userid' => $user->id, 'reactforumid' => $reactforumoff->id]));
        $this->assertFalse($DB->record_exists('reactforum_track_prefs', ['userid' => $user->id, 'reactforumid' => $reactforumon->id]));
    }
    ...
    public function test_user_read_posts() {
        ...
        $reactforum4 = $this->getDataGenerator()->create_module('reactforum', ['course' => $course->id]);
        $cm4 = get_coursemodule_from_instance('reactforum', $reactforum4->id);
        $context4 = \context_module::instance($cm4->id);

        list($author, $user) = $this->helper_create_users($course, 2);

        list($f1d1, $f1p1) = $this->helper_post_to_reactforum($reactforum1, $author);
        $f1p1reply = $this->helper_post_to_discussion($reactforum1, $f1d1, $author);
        $f1d1 = $DB->get_record('reactforum_discussions', ['id' => $f1d1->id]);
        list($f1d2, $f1p2) = $this->helper_post_to_reactforum($reactforum1, $author);

        list($f2d1, $f2p1) = $this->helper_post_to_reactforum($reactforum2, $author);
        $f2p1reply = $this->helper_post_to_discussion($reactforum2, $f2d1, $author);
        $f2d1 = $DB->get_record('reactforum_discussions', ['id' => $f2d1->id]);
        list($f2d2, $f2p2) = $this->helper_post_to_reactforum($reactforum2, $author);

        list($f3d1, $f3p1) = $this->helper_post_to_reactforum($reactforum3, $author);
        $f3p1reply = $this->helper_post_to_discussion($reactforum3, $f3d1, $author);
        $f3d1 = $DB->get_record('reactforum_discussions', ['id' => $f3d1->id]);
        list($f3d2, $f3p2) = $this->helper_post_to_reactforum($reactforum3, $author);

        list($f4d1, $f4p1) = $this->helper_post_to_reactforum($reactforum4, $author);
        $f4p1reply = $this->helper_post_to_discussion($reactforum4, $f4d1, $author);
        $f4d1 = $DB->get_record('reactforum_discussions', ['id' => $f4d1->id]);
        list($f4d2, $f4p2) = $this->helper_post_to_reactforum($reactforum4, $author);

        // Insert read info.
        // User has read post1, but not the reply or second post in reactforum1.
        reactforum_tp_add_read_record($user->id, $f1p1->id);

        // User has read post1 and its reply, but not the second post in reactforum2.
        reactforum_tp_add_read_record($user->id, $f2p1->id);
        reactforum_tp_add_read_record($user->id, $f2p1reply->id);

        // User has read post2 in reactforum3.
        reactforum_tp_add_read_record($user->id, $f3p2->id);

        // Nothing has been read in reactforum4.

        // Run as the user under test.
        $this->setUser($user);

        // Retrieve all contexts - should be three - reactforum4 has no data.
        $contextlist = $this->get_contexts_for_userid($user->id, 'mod_reactforum');
        $this->assertCount(3, $contextlist);
        ...
        $readdata = $writer->get_metadata(
                $this->get_subcontext($reactforum1, $f1d1, $f1p1),
                'postread'
            );
        ...
        $readdata = $writer->get_metadata(
                $this->get_subcontext($reactforum1, $f1d1, $f1p1reply),
                'postread'
            );
        ...
        // User has not f1p2.
        $readdata = $writer->get_metadata(
                $this->get_subcontext($reactforum1, $f1d2, $f1p2),
                'postread'
            );
        $this->assertEmpty($readdata);
        ...
        // User has read f2p1.
        $readdata = $writer->get_metadata(
                $this->get_subcontext($reactforum2, $f2d1, $f2p1),
                'postread'
            );
        $this->assertNotEmpty($readdata);
        $this->assertTrue(isset($readdata->firstread));
        ...
        $readdata = $writer->get_metadata(
                $this->get_subcontext($reactforum2, $f2d1, $f2p1reply),
                'postread'
            );
        $this->assertNotEmpty($readdata);
        $this->assertTrue(isset($readdata->firstread));
        ...
        $readdata = $writer->get_metadata(
                $this->get_subcontext($reactforum2, $f2d2, $f2p2),
                'postread'
            );
        $this->assertEmpty($readdata);

        // ReactForum 3.
        $this->export_context_data_for_user($user->id, $context3, 'mod_reactforum');
        $writer = \core_privacy\local\request\writer::with_context($context3);

        // User has not read f3p1.
        $readdata = $writer->get_metadata(
                $this->get_subcontext($reactforum3, $f3d1, $f3p1),
                'postread'
            );
        $this->assertEmpty($readdata);
        ...
        $readdata = $writer->get_metadata(
                $this->get_subcontext($reactforum3, $f3d1, $f3p1reply),
                'postread'
            );
        $this->assertEmpty($readdata);
        ...
        // User has read f3p2.
        $readdata = $writer->get_metadata(
                $this->get_subcontext($reactforum3, $f3d2, $f3p2),
                'postread'
            );
        $this->assertNotEmpty($readdata);
        $this->assertTrue(isset($readdata->firstread));
        ...
        // Delete all data for one of the users in one of the reactforums.
        $approvedcontextlist = new \core_privacy\tests\request\approved_contextlist(
            \core_user::get_user($user->id),
            'mod_reactforum',
            [$context3->id]
        ...
        $this->assertTrue($DB->record_exists('reactforum_read', ['userid' => $user->id, 'reactforumid' => $reactforum1->id]));
        $this->assertTrue($DB->record_exists('reactforum_read', ['userid' => $user->id, 'reactforumid' => $reactforum2->id]));
        $this->assertTrue($DB->record_exists('reactforum_read', ['userid' => $user->id, 'reactforumid' => $reactforum3->id]));

        provider::delete_data_for_user($approvedcontextlist);

        $this->assertTrue($DB->record_exists('reactforum_read', ['userid' => $user->id, 'reactforumid' => $reactforum1->id]));
        $this->assertTrue($DB->record_exists('reactforum_read', ['userid' => $user->id, 'reactforumid' => $reactforum2->id]));
        $this->assertFalse($DB->record_exists('reactforum_read', ['userid' => $user->id, 'reactforumid' => $reactforum3->id]));
    }
    ...
    public function test_post_attachment_inclusion() {
        ...
        // Create a new discussion + post in the reactforum.
        ...
        \core_tag_tag::set_item_tags('mod_reactforum', 'reactforum_posts', $post->id, $context, ['example', 'tag']);
        \core_tag_tag::set_item_tags('mod_reactforum', 'reactforum_posts', $reply->id, $context, ['example', 'differenttag']);

        // Create a second discussion + post in the reactforum without tags.
        list($otherdiscussion, $otherpost) = $this->helper_post_to_reactforum($reactforum, $author);
        $otherdiscussion = $DB->get_record('reactforum_discussions', ['id' => $otherdiscussion->id]);
        ...
        $contextlist = $this->get_contexts_for_userid($author->id, 'mod_reactforum');
        $this->assertCount(1, $contextlist);

        $this->export_context_data_for_user($author->id, $context, 'mod_reactforum');
        $writer = \core_privacy\local\request\writer::with_context($context);

        // The inline file should be on the first reactforum post.
        $subcontext = $this->get_subcontext($reactforum, $discussion, $post);
    ...
    public function test_post_tags() {
        ...
        $this->export_all_data_for_user($author->id, 'mod_reactforum');
        $writer = \core_privacy\local\request\writer::with_context($context);
    ...
    public function test_all_users_deleted_from_context() {
        ...
        for ($i = 0; $i < 2; $i++) {
            ...
            $contexts[$reactforum->id] = $context;
        }
        ...
        foreach ($users as $user) {
            foreach ($reactforums as $reactforum) {
                $context = $contexts[$reactforum->id];
        ...
        // Delete for the first reactforum.
        $reactforum = reset($reactforums);
        $context = $contexts[$reactforum->id];
        provider::delete_data_for_all_users_in_context($context);

        // Determine what should have been deleted.
        $discussionsinreactforum = array_filter($discussions, function($discussion) use ($reactforum) {
            return $discussion->reactforum == $reactforum->id;
        });

        $postsinreactforum = array_filter($posts, function($post) use ($discussionsinreactforum) {
            return isset($discussionsinreactforum[$post->discussion]);
        });

        // All reactforum discussions and posts should have been deleted in this reactforum.
        $this->assertCount(0, $DB->get_records('reactforum_discussions', ['reactforum' => $reactforum->id]));

        list ($insql, $inparams) = $DB->get_in_or_equal(array_keys($discussionsinreactforum));
        $this->assertCount(0, $DB->get_records_select('reactforum_posts', "discussion {$insql}", $inparams));

        // All uploaded files relating to this context should have been deleted (post content).
        foreach ($postsinreactforum as $post) {
            $this->assertEmpty($fs->get_area_files($context->id, 'mod_reactforum', 'post', $post->id));
            $this->assertEmpty($fs->get_area_files($context->id, 'mod_reactforum', 'attachment', $post->id));
        }
        ...
        $posttags = \core_tag_tag::get_items_tags('mod_reactforum', 'reactforum_posts', array_keys($postsinreactforum));
        foreach ($posttags as $tags) {
            $this->assertEmpty($tags);
        }

        // Check the other reactforum too. It should remain intact.
        $reactforum = next($reactforums);
        $context = $contexts[$reactforum->id];

        // Grab the list of discussions and posts in the reactforum.
        $discussionsinreactforum = array_filter($discussions, function($discussion) use ($reactforum) {
            return $discussion->reactforum == $reactforum->id;
        });
        ...
        // ReactForum discussions and posts should not have been deleted in this reactforum.
        $this->assertGreaterThan(0, $DB->count_records('reactforum_discussions', ['reactforum' => $reactforum->id]));

        list ($insql, $inparams) = $DB->get_in_or_equal(array_keys($discussionsinreactforum));
        $this->assertGreaterThan(0, $DB->count_records_select('reactforum_posts', "discussion {$insql}", $inparams));

        // Uploaded files relating to this context should remain.
        foreach ($postsinreactforum as $post) {
            if ($post->parent == 0) {
                $this->assertNotEmpty($fs->get_area_files($context->id, 'mod_reactforum', 'post', $post->id));
            }
        }
    ...
    public function test_delete_data_for_user() {
        ...
        foreach ($users as $user) {
            ...
            foreach ($reactforums as $reactforum) {
                ...
                $discussion = $DB->get_record('reactforum_discussions', ['id' => $discussion->id]);
                $discussions[$discussion->id] = $discussion;
                $postsbyreactforum[$user->id][$context->id] = [];

                // Add a number of replies.
                $posts[$post->id] = $post;
                $thisreactforumposts[$post->id] = $post;
                $postsbyreactforum[$user->id][$context->id][$post->id] = $post;

                $reply = $this->helper_reply_to_post($post, $user);
                $posts[$reply->id] = $reply;
                $postsbyreactforum[$user->id][$context->id][$reply->id] = $reply;
        ...
        foreach ($posts as $post) {
            ...
            reactforum_tp_add_read_record($user1->id, $post->id);
        }
        ...
        // Delete for one of the reactforums for the first user.
        $firstcontext = reset($contexts);

        $deletedpostids = [];
        $otherpostids = [];
        foreach ($postsbyreactforum as $user => $contexts) {
        ...
        $this->assertCount(40, $DB->get_records('reactforum_posts'));

        // There should be 8 posts belonging to user1.
        $this->assertCount(8, $DB->get_records('reactforum_posts', [
                'userid' => $user1->id,
            ]));
        ...
        $this->assertCount(4, $DB->get_records_select('reactforum_posts', "userid = :userid AND deleted = :deleted"
        ...
        $this->assertCount(4, $DB->get_records('reactforum_posts', [
                'deleted' => 1,
            ]));
        $this->assertCount(4, $DB->get_records_select('reactforum_posts',
        ...
        // 5 users * 2 reactforums * 1 file in each reactforum
        // Original total: 10
    ...
    public function test_delete_data_for_users() {
        ...
        $approveduserlist = new \core_privacy\local\request\approved_userlist($firstcontext, 'mod_reactforum', [$user1->id]);
    ...
    public function test_get_users_in_context_post_author() {
        ...
        list($fd1, $fp1) = $this->helper_post_to_reactforum($reactforum, $author);

        $userlist = new \core_privacy\local\request\userlist($context, $component);
        \mod_reactforum\privacy\provider::get_users_in_context($userlist);
    ...
    public function test_get_users_in_context_post_authors() {
        ...
        list($fd1, $fp1) = $this->helper_post_to_reactforum($reactforum, $author);
        $fp1reply = $this->helper_post_to_discussion($reactforum, $fd1, $user);
        $fd1 = $DB->get_record('reactforum_discussions', ['id' => $fd1->id]);
    ...
    public function test_get_users_in_context_post_ratings() {
        ...
        $fp1reply = $this->helper_post_to_discussion($reactforum, $fd1, $author);
        $fd1 = $DB->get_record('reactforum_discussions', ['id' => $fd1->id]);
    ...
    public function test_get_users_in_context_digest_preference() {
        ...
        $otherreactforum = $this->getDataGenerator()->create_module('reactforum', ['course' => $course->id]);
        $othercm = get_coursemodule_from_instance('reactforum', $otherreactforum->id);
        $othercontext = \context_module::instance($othercm->id);

        list($user, $otheruser) = $this->helper_create_users($course, 2);

        // Add digest subscriptions.
        reactforum_set_user_maildigest($reactforum, 0, $user);
        reactforum_set_user_maildigest($otherreactforum, 0, $otheruser);
    ...
    /**
     * Ensure that all users with a reactforum subscription preference included as a user in the context.
     */
    ...
    public function test_get_users_in_context_with_discussion_subscription() {
        ...
        // Post in both of the reactforums.
        list($fd1, $fp1) = $this->helper_post_to_reactforum($reactforum, $author);
        list($ofd1, $ofp1) = $this->helper_post_to_reactforum($otherreactforum, $author);

        // Subscribe the user to the discussions.
        \mod_reactforum\subscriptions::subscribe_user_to_discussion($user->id, $fd1);
        \mod_reactforum\subscriptions::subscribe_user_to_discussion($otheruser->id, $ofd1);
    ...
    public function test_get_users_in_context_with_read_post_tracking() {
        ...
        reactforum_tp_add_read_record($user->id, $fp1->id);
        reactforum_tp_add_read_record($otheruser->id, $ofp1->id);
    ...
    public function test_get_users_in_context_with_tracking_preferences() {
        ...
        // ReactForum tracking is opt-out.
        // Stop tracking the read posts.
        reactforum_tp_stop_tracking($reactforum->id, $user->id);
        reactforum_tp_stop_tracking($otherreactforum->id, $otheruser->id);

        $userlist = new \core_privacy\local\request\userlist($context, $component);
        \mod_reactforum\privacy\provider::get_users_in_context($userlist);

        // One user - the one who is tracking that reactforum.
        $this->assertCount(1, $userlist);
