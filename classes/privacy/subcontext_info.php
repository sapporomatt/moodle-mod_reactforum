...
trait subcontext_info {
    ...
    protected static function get_discussion_area(\stdClass $discussion) : Array {
        ...
        $pathparts[] = get_string('discussions', 'mod_reactforum');
    ...
    /**
     * Get the parent subcontext for the supplied reactforum, discussion, and post combination.
     *
     * @param   \stdClass   $post The post.
     * @return  array
     */
    protected static function get_post_area_for_parent(\stdClass $post) {
        ...
        $subcontext = [];
        if ($parent = $DB->get_record('reactforum_posts', ['id' => $post->parent], 'id, created, subject')) {
    ...
    /**
     * Get the subcontext for the supplied reactforum, discussion, and post combination.
     *
     ...
     * @return  array
     */
    protected static function get_subcontext($reactforum, $discussion = null, $post = null) {
        $subcontext = [];
        if (null !== $discussion) {
            $subcontext += self::get_discussion_area($discussion);

            if (null !== $post) {
                $subcontext[] = get_string('posts', 'mod_reactforum');
