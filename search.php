...
$reactforumid = trim(optional_param('reactforumid', 0, PARAM_INT));      // ReactForumID to search for
...
if (empty($search)) {   // Check the other parameters instead
    ...
    if (!empty($reactforumid)) {
        $search .= ' reactforumid:'.$reactforumid;
    }
    if (!empty($user)) {
        $search .= ' '.reactforum_clean_search_terms($user, 'user:');
    }
    if (!empty($subject)) {
        $search .= ' '.reactforum_clean_search_terms($subject, 'subject:');
    }
    if (!empty($fullwords)) {
        $search .= ' '.reactforum_clean_search_terms($fullwords, '+');
    }
    if (!empty($notwords)) {
        $search .= ' '.reactforum_clean_search_terms($notwords, '-');
    }
    if (!empty($phrase)) {
        $search .= ' "'.$phrase.'"';
    }
...
if ($search) {
    $search = reactforum_clean_search_terms($search);
}
...
$strsearch = get_string("search", "reactforum");
$strsearchresults = get_string("searchresults", "reactforum");
$strpage = get_string("page");

if (!$search || $showform) {

    $PAGE->navbar->add($strreactforums, new moodle_url('/mod/reactforum/index.php', array('id'=>$course->id)));
    $PAGE->navbar->add(get_string('advancedsearch', 'reactforum'));

    $PAGE->set_title($strsearch);
    $PAGE->set_heading($course->fullname);
    echo $OUTPUT->header();

    reactforum_print_big_search_form($course);
    echo $OUTPUT->footer();
    exit;
}
...
$searchterms = str_replace('reactforumid:', 'instance:', $search);
$searchterms = explode(' ', $searchterms);

$searchform = reactforum_search_form($course, $search);

$PAGE->navbar->add($strsearch, new moodle_url('/mod/reactforum/search.php', array('id'=>$course->id)));
$PAGE->navbar->add($strsearchresults);
if (!$posts = reactforum_search_posts($searchterms, $course->id, $page*$perpage, $perpage, $totalcount)) {
    $PAGE->set_title($strsearchresults);
    $PAGE->set_heading($course->fullname);
    echo $OUTPUT->header();
    echo $OUTPUT->heading($strreactforums, 2);
    echo $OUTPUT->heading($strsearchresults, 3);
    echo $OUTPUT->heading(get_string("noposts", "reactforum"), 4);
...
foreach ($posts as $post) {
    ...
    // Replace the simple subject with the three items reactforum name -> thread name -> subject
    ...
    $fullsubject = "<a href=\"view.php?f=$reactforum->id\">".format_string($reactforum->name,true)."</a>";
    ...
    //Unfortunately seem to have do this individually as posts may be from different reactforums
    ...
    // the start of the message text. However, do not do the highlighting here. reactforum_print_post
    ...
    if ($missing_terms) {
        $strmissingsearchterms = get_string('missingsearchterms','reactforum');
        $post->message = '<p class="highlight2">'.$strmissingsearchterms.' '.$missing_terms.'</p>'.$post->message;
    }

    // Prepare a link to the post in context, to be displayed after the reactforum post.
    $fulllink = "<a href=\"discuss.php?d=$post->discussion#p$post->id\">".get_string("postincontext", "reactforum")."</a>";
...
function reactforum_print_big_search_form($course) {
    global $PAGE, $words, $subject, $phrase, $user, $fullwords, $notwords, $datefrom, $dateto, $reactforumid;
 ...
 */
function reactforum_clean_search_terms($words, $prefix='') {
 ...
 /**
  * Retrieve a list of the reactforums that this user can view.
  *
  * @param stdClass $course The Course to use.
  * @return array A set of formatted reactforum names stored against the reactforum id.
  */
function reactforum_menu_list($course)  {
    $menu = array();
    ...
    foreach ($modinfo->instances['reactforum'] as $cm) {
        if (!$cm->uservisible) {
            continue;
        }
