...
$url = new moodle_url('/mod/reactforum/user.php', array('id' => $userid));
if ($isspecificcourse) {
...
if ($isspecificcourse) {
    ...
    $courses = reactforum_get_courses_user_posted_in($user, $discussionsonly);
}
...
$result = reactforum_get_posts_by_user($user, $courses, $isspecificcourse, $discussionsonly, ($page * $perpage), $perpage);
...
if (empty($result->posts)) {
    ...
    $pagetitle = get_string('noposts', 'mod_reactforum');

    // Get the page heading
    if ($isspecificcourse) {
        $pageheading = format_string($course->fullname, true, array('context' => $coursecontext));
    } else {
        $pageheading = get_string('pluginname', 'mod_reactforum');
    }
    ...
    if ($iscurrentuser) {
        ...
        // current user.
        if ($discussionsonly) {
            $notification = get_string('nodiscussionsstartedbyyou', 'reactforum');
        } else {
            $notification = get_string('nopostsmadebyyou', 'reactforum');
        }
        // These are the user's reactforum interactions.
        ...
        if (isset($courseid) && $courseid != SITEID) {
            ...
            if ($mode == 'posts') {
                $navbar = $PAGE->navbar->add(get_string('posts', 'reactforum'), new moodle_url('/mod/reactforum/user.php',
                        array('id' => $user->id, 'course' => $courseid)));
            } else {
                $navbar = $PAGE->navbar->add(get_string('discussions', 'reactforum'), new moodle_url('/mod/reactforum/user.php',
        ...
        if ($discussionsonly) {
            $notification = get_string('nodiscussionsstartedby', 'reactforum', $fullname);
        } else {
            $notification = get_string('nopostsmadebyuser', 'reactforum', $fullname);
        }
    } else {
        // Don't extend the navigation it would be giving out information that
        // the current uesr doesn't have access to.
        $notification = get_string('cannotviewusersposts', 'reactforum');
        if ($isspecificcourse) {
...
$discussions = $DB->get_records_list('reactforum_discussions', 'id', array_unique($discussions));

//todo Rather than retrieving the ratings for each post individually it would be nice to do them in groups
//however this requires creating arrays of posts with each array containing all of the posts from a particular reactforum,
...
foreach ($result->posts as $post) {
    if (!isset($result->reactforums[$post->reactforum]) || !isset($discussions[$post->discussion])) {
        // Something very VERY dodgy has happened if we end up here
        continue;
    }
    $reactforum = $result->reactforums[$post->reactforum];
    $cm = $reactforum->cm;
    $discussion = $discussions[$post->discussion];
    $course = $result->courses[$discussion->course];

    $reactforumurl = new moodle_url('/mod/reactforum/view.php', array('id' => $cm->id));
    $discussionurl = new moodle_url('/mod/reactforum/discuss.php', array('d' => $post->discussion));
    ...
    if ($reactforum->assessed != RATING_AGGREGATE_NONE) {
        ...
        if ($reactforum->type == 'single' or !$post->discussion) {
            $ratingoptions->returnurl = $reactforumurl;
        } else {
    ...
    $reactforumname = format_string($reactforum->name, true, array('context' => $cm->context));
    ...
    if (!$isspecificcourse && !$hasparentaccess) {
        ...
        $fullsubjects[] = html_writer::link($reactforumurl, $reactforumname);
    } else {
        $fullsubjects[] = html_writer::tag('span', $courseshortname);
        $fullsubjects[] = html_writer::tag('span', $reactforumname);
    }
    if ($reactforum->type != 'single') {
        ...
        if ($post->parent != 0) {
            ...
            if (!$isspecificcourse && !$hasparentaccess) {
                $fullsubjects[] .= html_writer::link(new moodle_url('/mod/reactforum/discuss.php', array('d' => $post->discussion, 'parent' => $post->id)), $postname);
            } else {
    ...
    $fulllink = html_writer::link($discussionurl, get_string("postincontext", "reactforum"));

    $postoutput[] = reactforum_print_post($post, $discussion, $reactforum, $cm, $course, false, false, false, $fulllink, '', null, true, null, true);
}

$userfullname = fullname($user);

if ($discussionsonly) {
    $inpageheading = get_string('discussionsstartedby', 'mod_reactforum', $userfullname);
} else {
    $inpageheading = get_string('postsmadebyuser', 'mod_reactforum', $userfullname);
}
if ($isspecificcourse) {
    $a = new stdClass;
    ...
    if ($discussionsonly) {
        $pagetitle = get_string('discussionsstartedbyuserincourse', 'mod_reactforum', $a);
    } else {
        $pagetitle = get_string('postsmadebyuserincourse', 'mod_reactforum', $a);
    }
} else {
...
if (!empty($postoutput)) {
    ...
    echo $OUTPUT->heading(get_string('nodiscussionsstartedby', 'reactforum', $userfullname));
} else {
    echo $OUTPUT->heading(get_string('noposts', 'reactforum'));
}
