...
class mod_reactforum_renderer extends plugin_renderer_base {
    ...
    public function neighbouring_discussion_navigation($prev, $next) {
        ...
        if ($prev || $next) {
            ...
            if ($prev) {
                $url = new moodle_url('/mod/reactforum/discuss.php', array('d' => $prev->id));
                $html .= html_writer::start_tag('li', array('class' => 'prev-discussion'));
                $html .= html_writer::link($url, format_string($prev->name),
                    array('aria-label' => get_string('prevdiscussiona', 'mod_reactforum', format_string($prev->name))));
                $html .= html_writer::end_tag('li');
            }
            if ($next) {
                $url = new moodle_url('/mod/reactforum/discuss.php', array('d' => $next->id));
                $html .= html_writer::start_tag('li', array('class' => 'next-discussion'));
                $html .= html_writer::link($url, format_string($next->name),
                    array('aria-label' => get_string('nextdiscussiona', 'mod_reactforum', format_string($next->name))));
                $html .= html_writer::end_tag('li');
            }
     ...
     * @return string
     */
    public function subscriber_overview($users, $reactforum , $course) {
        $output = '';
        $modinfo = get_fast_modinfo($course);
        if (!$users || !is_array($users) || count($users)===0) {
            $output .= $this->output->heading(get_string("nosubscribers", "reactforum"));
        } else if (!isset($modinfo->instances['reactforum'][$reactforum->id])) {
            ...
            $strparams->name = format_string($reactforum->name);
            $strparams->count = count($users);
            $output .= $this->output->heading(get_string("subscriberstowithcount", "reactforum", $strparams));
            $table = new html_table();
            $table->cellpadding = 5;
            $table->cellspacing = 5;
    ...
    public function subscribed_users(user_selector_base $existingusers) {
        ...
        $output .= html_writer::tag('p', get_string('forcesubscribed', 'reactforum'));
    ...
    public function timed_discussion_tooltip($discussion, $visiblenow) {
        ...
        if ($discussion->timestart) {
            $dates[] = get_string('displaystart', 'mod_reactforum').': '.userdate($discussion->timestart);
        }
        if ($discussion->timeend) {
            $dates[] = get_string('displayend', 'mod_reactforum').': '.userdate($discussion->timeend);
        }

        $str = $visiblenow ? 'timedvisible' : 'timedhidden';
        $dates[] = get_string($str, 'mod_reactforum');
    ...
    /**
     * Display a reactforum post in the relevant context.
     *
     * @param \mod_reactforum\output\reactforum_post $post The post to display.
     * @return string
     */
    public function render_reactforum_post_email(\mod_reactforum\output\reactforum_post_email $post) {
        $data = $post->export_for_template($this, $this->target === RENDERER_TARGET_TEXTEMAIL);
        return $this->render_from_template('mod_reactforum/' . $this->reactforum_post_template(), $data);
    }
